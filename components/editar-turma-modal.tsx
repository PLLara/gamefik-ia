...
interface EditarTurmaModalProps {
  ...
  onClose: () => void
  onSave?: (data: { id: string; nome: string; segmento: string }) => void
}

export function EditarTurmaModal({ turma, onClose, onSave }: EditarTurmaModalProps) {
  const [nome, setNome] = useState(turma.nome)
  const [segmento, setSegmento] = useState(turma.segmento)
  const [showSegmentoDropdown, setShowSegmentoDropdown] = useState(false)

  const isValid = nome.trim() !== "" && segmento !== ""
  const hasChanges = nome.trim() !== turma.nome || segmento !== turma.segmento

  function handleSave() {
    if (isValid && onSave) {
      onSave({ id: turma.id, nome: nome.trim(), segmento })
    }
    onClose()
  }
