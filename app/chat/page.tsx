...
function ChatContent() {
  ...
  const addQuestion = () => {
    ...
    const newLabel = String.fromCharCode(65) // "A"
  ...
  const addAlternative = (qIndex: number) => {
    setQuizQuestions(prev => {
      const copy = [...prev]
      const q = copy[qIndex]
      const nextLabel = String.fromCharCode(65 + q.alternatives.length) // A=65, B=66, etc.
      if (q.alternatives.length >= 6) return prev // max 6 alternatives
      copy[qIndex] = {
        ...q,
  ...
  const removeAlternative = (qIndex: number, altIndex: number) => {
    setQuizQuestions(prev => {
      const copy = [...prev]
      const q = copy[qIndex]
      if (q.alternatives.length <= 2) return prev // min 2 alternatives
  ...
  const renderMissionEditor = () => {
    return (
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-6 border-b border-border px-6 pt-1">
          <button
            className="border-b-2 border-primary pb-3 pt-2 text-sm font-medium text-primary"
          >
            Detalhes
          </button>
        </div>
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] flex-col overflow-hidden bg-background">
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <div className="hidden w-52 shrink-0 flex-col border-r border-border bg-card lg:flex">
          <div className="p-3">
            <button onClick={handleBack} className="flex w-full items-center justify-center gap-2 rounded-lg border border-primary/30 bg-primary/5 px-4 py-2.5 text-sm font-medium text-primary transition-colors hover:bg-primary/10">
              <Plus className="h-4 w-4" />
              Nova atividade
            </button>
          </div>
          <div className="px-3 pb-3">
            <div className="relative">
              ...
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Buscar conversas..." className="h-9 w-full rounded-lg border border-border bg-background pl-9 pr-3 text-sm outline-none placeholder:text-muted-foreground focus:border-ring focus:ring-2 focus:ring-ring/30" />
            </div>
          </div>
          <div className="flex-1 overflow-auto px-2">
            <p className="px-2 py-2 text-xs font-medium uppercase tracking-wider text-muted-foreground">Recentes</p>
            <div className="space-y-0.5">
              {recentActivities.map((activity) => (
                <button key={activity.id} className="flex w-full items-start gap-2.5 rounded-lg px-2.5 py-2 text-left transition-colors hover:bg-sidebar-accent">
                  {activity.type === "quiz" ? <Gamepad2 className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" /> : <ClipboardList className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />}
        ...
        {/* Center - Chat Area */}
        <div className="flex w-[360px] shrink-0 flex-col border-r border-border xl:w-[400px]">
          <div className="flex-1 overflow-auto p-4">
            <div className="mx-auto max-w-2xl space-y-4">
              {messages.map((msg) => (
                <div key={msg.id} className={cn("flex gap-2", msg.role === "user" && "flex-row-reverse")}>
                  {msg.role === "ai" && (
                    <div className="relative h-7 w-7 shrink-0 overflow-hidden rounded-full bg-gradient-to-br from-amber-100 to-orange-100">
                      <img 
                        src="/images/characters.png" 
                        alt="Assistente IA" 
                        className="absolute -top-0.5 left-1/2 h-8 w-auto -translate-x-1/2 object-cover"
                        style={{ objectPosition: 'center 10%' }}
                      />
                    </div>
                  )}
                  <div className={cn("max-w-[85%] rounded-2xl px-4 py-3 text-sm", msg.role === "ai" ? "bg-muted text-foreground" : "bg-primary text-primary-foreground")}>
                    {msg.content}
                  </div>
                </div>
              ))}
          ...
          <div className="border-t border-border bg-card/50 px-4 py-3">
            <div className="mx-auto flex max-w-2xl flex-wrap gap-2">
              {quickChips.map((chip) => (
                <button key={chip} onClick={() => handleQuickChip(chip)} className="rounded-full border border-border bg-card px-3 py-1.5 text-xs font-medium text-foreground transition-colors hover:bg-sidebar-accent">
                  {chip}
                </button>
              ))}
            </div>
          ...
          <div className="border-t border-border bg-card p-4">
            <div className="mx-auto max-w-2xl">
              <div className="flex items-end gap-3 rounded-xl border border-border bg-background p-2">
                <div className="flex gap-1">
                  <button type="button" onClick={() => fileInputRef.current?.click()} className="flex h-9 w-9 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-sidebar-accent hover:text-foreground">
                ...
                <textarea
                  ...
                  rows={1}
                  className="flex-1 resize-none bg-transparent py-2 text-sm text-foreground outline-none placeholder:text-muted-foreground"
                  onKeyDown={(e) => {
                    if (e.key === "Enter" && !e.shiftKey) {
                      e.preventDefault()
                      handleSubmit(e)
...
<button 
  ...
  disabled={!canSubmit} 
  className={cn(
    "group relative flex h-10 w-10 items-center justify-center rounded-xl transition-all duration-300 ease-out",
    canSubmit 
    ...
    canSubmit && "group-hover:translate-x-0.5 group-hover:-translate-y-0.5"
        ...
        <div className="hidden flex-1 flex-col bg-muted/20 md:flex">
            ...
            <div className="flex items-center gap-2">
              {rightPanel === "preview" ? (
                ...
                <>
                  ...
                  <span className="text-sm font-medium text-foreground">Editor de atividade</span>
                </>
              )}
            </div>
          ...
          <div className="flex items-center justify-between border-t border-border bg-card px-6 py-4">
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                <button onClick={() => setShowClassDropdown(!showClassDropdown)} className="flex items-center gap-2 rounded-lg border border-border bg-card px-4 py-2.5 text-sm font-medium text-foreground transition-colors hover:bg-sidebar-accent">
                ...
                {showClassDropdown && (
                  <>
                    ...
                    <div className="absolute bottom-full right-0 z-50 mb-2 w-48 rounded-lg border border-border bg-card p-1 shadow-lg">
                      <button className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm text-foreground hover:bg-sidebar-accent">Todas as turmas</button>
                      <button className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm text-foreground hover:bg-sidebar-accent">6o Ano A</button>
                      <button className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm text-foreground hover:bg-sidebar-accent">7o Ano B</button>
                    </div>
                  </>
                )}
              </div>
              <button className="flex items-center gap-2 rounded-lg px-6 py-2.5 text-sm font-semibold text-white shadow-lg transition-opacity hover:opacity-90" style={{ background: "linear-gradient(to right, #67e8f9, #f0abfc, #fde047)" }}>
                <Sparkles className="h-4 w-4" />
                Publicar
              </button>
            </div>
